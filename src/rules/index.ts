import type { Rule, ParsedCommit } from '../types.js';
import { ensureCase, ensureEnum, maxLength, minLength, maxLineLength, notEmpty } from '../ensure.js';
import { xSync } from 'tinyexec';

export const bodyCase: Rule<string | string[]> = (parsed, when = 'always', value = []) => {
  if (!parsed.body) return [true];
  const list = Array.isArray(value) ? value : [value];
  const passes = when === 'always'
...
export const scopeDelimiterStyle: Rule<string[]> = (parsed, when = 'always', value = []) => {
  ...
  return [isNever ? !allowed : allowed, `scope delimiters must ${isNever ? 'not ' : ''}be one of [${delimiters.join(', ')}]`];
};
...
export const signedOffBy: Rule<string> = (parsed, when = 'always', value = 'Signed-off-by:') => {
  const lines = (parsed.raw || '')
    .split(/\r?\n/)
    .filter((ln) => !ln.startsWith('#') && !/^\(cherry picked from commit [0-9a-f]{7,64}\)$/i.test(ln.trim()) && Boolean(ln));
  const last = lines[lines.length - 1];
  const hasSignedOff = last ? last.startsWith(value) : false;
  const negated = when === 'never';
  return [negated ? !hasSignedOff : hasSignedOff, `message ${negated ? 'must not' : 'must'} be signed off`];
};
...
export const trailerExists: Rule<string> = (parsed, when = 'always', value = '') => {
  try {
    const res = xSync('git', ['interpret-trailers', '--parse'], { nodeOptions: { input: parsed.raw || '' } });
    const count = (res.stdout || '').split(/\r?\n/).filter((ln) => ln.startsWith(value)).length;
    const hasTrailer = count > 0;
    const negated = when === 'never';
    return [negated ? !hasTrailer : hasTrailer, `message ${negated ? 'must not' : 'must'} have "${value}" trailer`];
  } catch {
    return [true];
  }
};
